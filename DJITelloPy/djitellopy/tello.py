...
class Tello:
    ...
    def streamon(self):
        ...
        ビデオストリーミングをオンにします。その後、`tello.get_frame_read`を使用してください。
        ビデオストリーミングは、APモード（コンピュータがTello-XXXXX WiFi ntworkに接続されている場合）であれば、
        すべてのTelloでサポートされています。
        現在、Tello EDU は WiFi ネットワークに接続されている間はビデオストリーミングをサポートしていません。

        !!! 注：「Unknown command（不明なコマンド）」という応答があった場合、
            Tello ファームウェアをアップデートする必要があります。
            これは、Tello の公式アプリを使用して行うことができます。
        """
